...
int run_self_test() {
    ...
    // ── 銀行摘要（--bank）──
    {
        dm::NormOptions bank; bank.bank = true;
        auto nb = [&](const std::string& x) { return dm::normalize(x, bank); };
        eq(nb("ｶ)ﾐﾗｲﾊｲｿｳｻｰﾋﾞｽ"), "ミライハイソウサービス", "前株の略号");
        eq(nb("ﾐﾎﾝｾｷﾕｻｰﾋﾞｽ(ｶ"), "ミホンセキユサービス", "後株の略号");
        eq(nb("ﾕ)ﾔﾏﾄｳﾝﾕ"), "ヤマトウンユ", "有限会社の略号");
        eq(nb("ﾌﾘｺﾐ ｶ)ｻﾝﾌﾟﾙｼｮｳｼﾞ"), "サンプルショウジ", "取引種別語＋略号");
        eq(nb("ﾃﾞﾋﾞｯﾄ ｻﾝﾌﾟﾙﾏｰﾄ"), "サンプルマート", "デビットの語");
        eq(nb("シャ)ケンショウカイ"), "ケンショウカイ", "社団法人の略号（シ)と誤らない）");
        // 【重要】略号の除去は摘要の文脈でだけ行う。
        // 通常の正規化で「カ」で始まる名前が削れてはいけない。
        eq(dm::normalize("カ)ミライハイソウサービス"), "カ)ミライハイソウサービス",
           "通常モードでは略号を触らない");
        // 名前の中に略号と同じ文字があっても削れない
        eq(nb("ﾀｶﾗﾂｶｴｷﾏｴｼｮｳｼﾞ"), "タカラツカエキマエショウジ", "名前の中のカは残る");
    }
